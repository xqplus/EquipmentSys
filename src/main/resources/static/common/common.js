...
let deptContent = '<form class="layui-col-md11 layui-form" id="dialogAddForm">\n' +
    ...
    '        <div class="layui-input-block">\n' +
    '            <input id="deptNumber" type="text" readonly class="layui-input layui-disabled" name="deptNumber" lay-verify="required" placeholder="不可用,选择部门类型自动填充" autocomplete="off"/>\n' +
    '        </div>\n' +
    '    </div>\n' +
    '    <div class="layui-form-item">\n' +
    '        <label class="layui-form-label input-required">部门名称：</label>\n' +
    '        <div class="layui-input-block">\n' +
    '            <input type="text" id="deptName" readonly class="layui-input layui-disabled" name="deptName" lay-verify="required" placeholder="不可用,选择部门类型自动填充" autocomplete="off"/>\n' +
    '        </div>\n' +
    '    </div>\n' +
    '    <div class="layui-form-item">\n' +
...
let methods = {
     ...
     */
    passwordsInputCheck: function (pwdId1, pwdId2) {
    ...
    */
    userExistsCheck: function (selector, boolean) {
        $(selector).blur(function () {
            $.ajax({
                async: false,
                type: 'GET',
                url: methods.getUrl('/equipmentSys/user/userCheck'),
                data: {
     ...
     */
    formSubmitEvent: function (filter, url, boolean) {
     ...
     */
    roleDeptCascade: function (filter) {
        layui.form.on('select(' + filter + ')', function (data) {
            ...
            if (data.value !== '') {
                $.ajax({
                    async: false, // 异步提交
                    type: 'GET',
                    url: methods.getUrl('/equipmentSys/department/getDeptByRole'),
                    data: {
                        role: data.value
                    },
                    // dataType: 'json',
     ...
     */
    formDialog: function (title, content, userNameSelector, pwdId1, pwdId2, filter, saveUrl, type, data, tableName) {
        let currIndex = layer.open({
            ...
            , success: function () {
                ...
                if (type === 'addUser' || type === 'editUser') {
                    // 输入用户名校验
                    methods.userExistsCheck(userNameSelector, true);
                    // 输入密码校验
                    methods.passwordsInputCheck(pwdId1, pwdId2);
                    // 角色部门级联
                    methods.roleDeptCascade(filter);
                }
                if (type === 'addDept' || type === 'editDept') {
                    // 部门信息级联
                    methods.getNextDeptByRole();
                }
                if (type === 'editDept') {
                    // 编辑时提交用于数据检索
                    $('#deptId').val(data.id);
                ...
                if (type === 'addEquipType') {
                    // 将最新设备类型编号下一个显示在输入框
                    methods.getNextEquipTypeNumber();
                }
                ...
                if (type === 'addApply' || type === 'editApply') {
                    $('#userName').val(methods.getCurrentUserInfo().userName);
                    methods.roleDeptCascade(filter);
                    // 获取最新的申请编号
                    methods.getNextApplyNumberByDept();
                }
                if (type === 'editApply') {
     ...
     */
    getNextDeptByRole: function () {
        layui.form.on('select(roleTypeDept)', function (data) {
            if (data.value !== '') {
                $.ajax({
                    async: false, // 异步提交
                    type: 'GET',
                    url: methods.getUrl('/equipmentSys/department/getNextDeptByRole'),
                    data: {
                        role: data.value
                    },
                    // dataType: 'json',
     ...
     */
    getNextEquipNumber: function () {
        $.ajax({
            async: false, // 异步提交
            type: 'GET',
            url: methods.getUrl('/equipmentSys/equipment/getNextEquipNumber'),
            data: {},
            // dataType: 'json',
            success: function (data) {
                // 将对应信息显示在表单上
     ...
     */
    getNextEquipTypeNumber: function () {
        $.ajax({
            async: false, // 异步提交
            type: 'GET',
            url: methods.getUrl('/equipmentSys/equipmentType/getNextEquipTypeNumber'),
            data: {},
            // dataType: 'json',
            success: function (data) {
                // 将对应信息显示在表单上
     ...
     */
    getNextApplyNumberByDept: function () {
        layui.form.on('select(deptNumber)', function (data) {
            if (data.value !== '') {
                $.ajax({
                    async: false, // 异步提交
                    type: 'GET',
                    url: methods.getUrl('/equipmentSys/apply/getNextApplyNumberByDeptNumber'),
                    data: {
    ...
    getCurrentUserInfo: function () {
        ...
        $.ajax({
            async: false,
            type: 'GET',
            url: methods.getUrl('/equipmentSys/user/getCurrentUserInfo'),
            success: function (data) {
    ...
    setBadge: function () {
        $.ajax({
            async: false,
            type: 'GET',
            url: methods.getUrl('/equipmentSys/todoAndNotice/todo'),
            data: {},
            success: function (data) {
                if (data[0] > 0) { // 申请数
     ...
     * @param where 刷新表格传参
     */
    myAjax: function (type, url, requestData, successInfo, errorInfo, isReload, tableId, where) {
        $.ajax({
            async: false,
            type: type,
            url: url,
            ...
            success: function (data) {
                if (data === 'success') {
                    ...
                    if (isReload) {
                        methods.tableReload(tableId, where);
                    }
                }
                if (data === 'error') {
     ...
     * @param param 传入参数
     */
    exportExcel: function (confirmMsg, url, param) {
        if (!$.isArray(param)) {
            param = [param];
        }
     ...
     * @param where 传参
     */
    tableReload: function (tableId, where) {
